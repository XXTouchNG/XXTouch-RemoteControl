...
local server =
    ...
    protocols = {
        ["RC"] = function(ws)
            sys.toast("Remote Control Established", device.front_orien())
            local index = 5
            ev.Timer.new(
                function()
                    if index <= 0 then
                        ...
                        sys.toast("Remote Control Terminated", device.front_orien())
                        os.exit()
                    end
                    index = index - 1
                ...
                function(ws, message, opcode)
                    if opcode == websocket.TEXT then
                        ...
                        if jobj then
                            ...
                            elseif jobj.mode == "home_down" then
                                key.down(0x0C, 64)
                            elseif jobj.mode == "home_up" then
                                key.up(0x0C, 64)
                            elseif jobj.mode == "home" then
                                key.press(0x0C, 64)
                            elseif jobj.mode == "power_down" then
                                key.down(0x0C, 48)
                            elseif jobj.mode == "power_up" then
                                key.up(0x0C, 48)
                            ...
                            elseif jobj.mode == "volume_increment" then
                                key.press(0x0C, 0xE9)
                            elseif jobj.mode == "volume_decrement" then
                                key.press(0x0C, 0xEA)
